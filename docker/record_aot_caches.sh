...
# the runner's is Cucumber and JUnit. One cache could not satisfy both.
#
...
#
# The workload jar stays in the jars directory afterwards, so that the classpath
# recorded here is the one a kata builds later. Its contents are confined to the
# aotworkload package, which is what keeps Cucumber from finding this feature
# when a learner runs their own.
...
cp -r /tmp/throwaway_kata/* "${WORK_DIR}"
cd "${WORK_DIR}"

javac -cp "$(ls ${JARS_DIR}/*.jar | tr '\n' ':')" aotworkload/*.java
# The .feature goes into the jar beside the classes: Cucumber reads it as a
# classpath resource, so it has to travel with them.
jar --create --file "${WORKLOAD_JAR}" aotworkload
